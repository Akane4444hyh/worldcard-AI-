#!/usr/bin/env python3
"""Generate world-card — a single-HTML flashcard app."""

import json
import os

HTML_PATH = "world-card.html"
...
<style>
@import url('https://cdn.jsdelivr.net/npm/@fontsource/maple-mono@5.2.6/index.min.css');
...
.card-back.has-phrases {{ justify-content: flex-start; }}
.card-back .meaning {{
  font-size: 1.4rem;
  color: #e8e0f0;
  margin-bottom: 18px;
...
@media (max-width: 768px) {{
  ...
  .card-container {{ width: 90vw; max-width: 90vw; min-height: 300px; height: auto; }}
  ...
  <aside class="sidebar hidden" id="sidebar">
    <div class="sidebar-inner">
      ...
      <div class="sidebar-content">
        <div class="sidebar-header">未斬 <span id="remainingCount">{total}</span> · 已斬 <span id="slainCount">0</span></div>
        <input class="search-box" id="searchBox" placeholder="在目前分組中搜尋…" autocomplete="off">
        <div class="word-list" id="wordList"></div>
        <div class="batch-restore-bar" id="batchRestoreBar">
          <span class="select-all-text" id="selectAllSlain">全選</span>
          <span class="select-all-text" id="deselectAllSlain">取消</span>
          <div style="flex:1"></div>
          <button class="toggle-btn" id="restoreSelectedBtn" style="font-size:12px;padding:6px 14px;">恢復選中 (<span id="selectedSlainCount">0</span>)</button>
        </div>
        <div class="sidebar-footer" style="display:flex;flex-direction:column;gap:6px;">
          <button class="toggle-btn" id="restoreAllBtn">恢復全部已斬</button>
          <button class="toggle-btn" id="deleteGroupBtn" style="display:none;color:#e11d48;border-color:#e11d4820;">刪除目前分組</button>
          <button class="reset-btn" id="resetAllBtn">重置所有資料</button>
        </div>
      </div>
    </div>
  </aside>
  ...
  <main class="main">
    <div class="topbar">
      <div class="topbar-group">
        ...
        <button class="toggle-btn" id="shuffleBtn" title="打亂順序">打亂</button>
        <button class="toggle-btn" id="resetOrderBtn" title="恢復預設順序">恢復</button>
        <button class="toggle-btn" id="importBtn" title="匯入單字">匯入</button>
      </div>
      <div style="display:flex;align-items:center;gap:10px;">
        <button class="toggle-btn" id="themeBtn" title="切換亮色/暗色模式">亮色</button>
    ...
    <div class="card-container" id="cardContainer">
      <div class="empty-state" id="emptyState">
        ...
        <div class="empty-sub">點擊「匯入」按鈕新增單字，支援 .json / .txt 格式</div>
        <button class="toggle-btn" id="emptyImportBtn">開始匯入</button>
      </div>
    ...
    <div class="nav" id="navBar">
      <button class="nav-btn" id="prevBtn" title="上一個 (←)">←</button>
      <button class="nav-btn flip-btn" id="flipBtn" title="翻轉 (空格)">⟳</button>
      <button class="nav-btn" id="nextBtn" title="下一個 (→)">→</button>
      <button class="nav-btn slay-btn" id="slayBtn" title="斬殺此字 (K)">斬</button>
    </div>

    <div class="hint">← → 翻頁 · 空格/點擊 翻轉 · K 斬殺 · R 恢復順序 · 輸入即搜尋</div>

    <!-- Import modal -->
    <div class="modal-overlay" id="importModal">
      <div class="modal">
        <h3>匯入單字</h3>
        <div class="modal-desc">支援上傳 .json / .txt 檔案，或手動貼上內容</div>
        <input class="group-input" id="importGroupName" placeholder="分組名（必填，同名歸入已有組）" autocomplete="off" required>
        <div class="file-upload">
          <label>選擇檔案（.json / .txt）</label>
          <input type="file" id="importFileInput" accept=".json,.txt" multiple>
        </div>
        <textarea id="importTextarea" placeholder="abandon — v. 放棄&#10;abuse — v. 濫用, n. 虐待&#10;..." style="height:140px;"></textarea>
        <div class="modal-actions">
          <button class="toggle-btn" id="cancelImportBtn">取消</button>
          <button class="toggle-btn" id="confirmImportBtn" style="background:#7c3aed;border-color:#7c3aed;color:#fff;">匯入</button>
        </div>
      </div>
    </div>
    ...
    <div class="modal-overlay" id="resetModal">
      <div class="modal confirm-modal">
        <h3>重置所有資料</h3>
        <p>這將清除所有匯入的單字、分組、斬殺記錄和順序設定。<br>此操作不可撤銷。</p>
        <div class="modal-actions">
          <button class="toggle-btn" id="cancelResetBtn">取消</button>
          <button class="toggle-btn" id="confirmResetBtn" style="background:#e11d48;border-color:#e11d48;color:#fff;">確認重置</button>
        </div>
      </div>
    </div>
  </main>
...
function buildGroupTabs() {{
  ...
  for (const g of groups) {{
    ...
    html += `<div class="group-tab-v${{activeCls}}" data-group="${{g.replace(/"/g, '&quot;')}}">
      ...
      <div class="gt-counts">未斬 ${{c.unslawn}}<br>已斬 ${{c.slawn}}</div>
    </div>`;
  }}
  ...
  delBtn.textContent = '刪除分組: ' + activeGroup;
}}
...
function buildWordList(filter = '') {{
  ...
  wordList.innerHTML = seq
    .map((ci, displayPos) => {{
      ...
      return `<div class="${{cls}}" data-card-idx="${{ci}}" data-order-idx="${{groupPos}}" data-display-pos="${{displayPos}}" ${{display}}>${{cbHtml}}${{c.word}}</div>`;
    }})
    .join('');
}}
...
async function doImport() {{
  try {{
    ...
    if (added > 0) {{
      ...
      card.classList.remove('flipped');
      renderCard();
      showToast('已匯入到「' + groupName + '」');
    }} else {{
      showToast('沒有新單字被匯入（可能已存在或格式不正確）');
    }}
    hideImportModal();
  }} catch(e) {{
...
wordList.addEventListener('mousedown', e => {{
  ...
  const dp = parseInt(item.dataset.displayPos);
  dragState = {{ startCi: ci, startDisplayPos: dp, startY: e.clientY, dragging: false, lastDisplayPos: dp }};
}});
...
wordList.addEventListener('mousemove', e => {{
  ...
  if (!item) return;
  const dp = parseInt(item.dataset.displayPos);
  if (dp === dragState.lastDisplayPos) return;

  // Compute range in display-position space (matches what the user sees)
  const fromDp = Math.min(dragState.startDisplayPos, dp);
  const toDp = Math.max(dragState.startDisplayPos, dp);
  const movingDown = dp >= dragState.startDisplayPos;
  ...
  const allItems = wordList.querySelectorAll('.word-item');
  for (const itemEl of allItems) {{
    const itemDp = parseInt(itemEl.dataset.displayPos);
    if (itemDp < fromDp || itemDp > toDp) continue;
    if (itemEl.style.display === 'none') continue;
    const ci = parseInt(itemEl.dataset.cardIdx);
    if (!slain.has(ci)) continue;
    ...
    if (movingDown) {{
      ...
      slainChecked.delete(ci);
      if (cb) {{ cb.classList.remove('checked'); cb.textContent = ''; }}
    }}
  }}
  dragState.lastDisplayPos = dp;
  updateBatchRestoreBar();
}});
